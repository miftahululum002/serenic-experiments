...
def analyze_log(filepath):
    ...
    start_pattern = re.compile(
        r"^(\S+)\s+INFO:.*\[endpoint=(\S+)(?: server=(\S+))? \(([^)]+)\)\] \[Worker \d+\] \[([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\] Start Processing INACBGS"
    )
    saved_pattern = re.compile(
        r"^(\S+)\s+INFO:.*\[endpoint=(\S+)(?: server=(\S+))? \(([^)]+)\)\] \[Worker \d+\] \[([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\] INACBGS saved \(set=(\w+)\)"
    )
    ...
    with open(fp) as f:
        for line in f:
            ...
            if m:
                ts, host, server, address, encounter = m.group(1), m.group(2), m.group(3), m.group(4), m.group(5)
                pending.setdefault(encounter, []).append((parse_timestamp(ts), host, server or "", address or ""))
                continue

            m = saved_pattern.match(line)
            if m:
                ts_str, host, server, address, encounter, set_type = m.group(1), m.group(2), m.group(3), m.group(4), m.group(5), m.group(6)
...
def print_summary(events):
    ...
    print(f"    Servers           : {sorted(set(e['server'] for e in events if e['server']))}")
    ...
    if durations:
        ...
        print(f"    Total duration    : {sum(durations):.1f}s ({sum(durations)/60:.2f} min)")
    ...
    if len(sets) > 1:
        print("\n  Per pass type:")
        print(f"    {'Set':<12} {'Cycles':<8} {'Avg (s)':<10} {'Min (s)':<10} {'Max (s)':<10} {'p95 (s)':<10}")
        for st in sets:
            sd = sorted(e["duration_seconds"] for e in events if e["set"] == st)
            n = len(sd)
            if n:
                avg = sum(sd) / n
                print(f"    {st:<12} {n:<8} {avg:<10.1f} {min(sd):<10.1f} {max(sd):<10.1f} {percentile(sd, 95):<10.1f}")

    print("\n  Per host:")
    hosts = sorted(set(e["host"] for e in events))
    if events:
        print(f"    {'Host':<12} {'Server':<22} {'IP':<22} {'Port':<6} {'Cycles':<8} {'Avg (s)':<10} {'Min (s)':<10} {'Max (s)':<10} {'p95 (s)':<10}")
        for host in hosts:
            hd = sorted(e["duration_seconds"] for e in events if e["host"] == host)
            n = len(hd)
            if n:
                avg = sum(hd) / n
                servers = sorted(set(e["server"] for e in events if e["host"] == host and e["server"]))
                ips = sorted(set(e["ip"] for e in events if e["host"] == host and e["ip"]))
                ports = sorted(set(e["port"] for e in events if e["host"] == host and e["port"]))
                print(f"    {host:<12} {','.join(servers) or '-':<22} {','.join(ips) or '-':<22} {','.join(ports) or '-':<6} {n:<8} {avg:<10.1f} {min(hd):<10.1f} {max(hd):<10.1f} {percentile(hd, 95):<10.1f}")

    print("\n  Per server:")
    server_groups = sorted(set((e["server"], e["port"]) for e in events if e["server"]))
    if events:
        print(f"    {'Server':<22} {'Port':<6} {'Hosts':<20} {'IP':<22} {'Cycles':<8} {'Avg (s)':<10} {'Min (s)':<10} {'Max (s)':<10} {'p95 (s)':<10}")
        for server, port in server_groups:
            sd = sorted(e["duration_seconds"] for e in events if e["server"] == server and e["port"] == port)
            n = len(sd)
            if n:
                avg = sum(sd) / n
                hosts = sorted(set(e["host"] for e in events if e["server"] == server and e["port"] == port))
                ips = sorted(set(e["ip"] for e in events if e["server"] == server and e["port"] == port and e["ip"]))
                print(f"    {server:<22} {port:<6} {','.join(hosts):<20} {','.join(ips) or '-':<22} {n:<8} {avg:<10.1f} {min(sd):<10.1f} {max(sd):<10.1f} {percentile(sd, 95):<10.1f}")
...
def analyze(filepath, count=0):
    ...
    header = (f"{'No':<5} {'Encounter ID':<38} {'Org ID':<38} {'Host':<12} {'Server':<22} {'IP':<22} {'Port':<6} {'Set':<10}"
              f" {'Start':<22} {'End':<22} {'Duration (s)':<14}")
    separator = (f"{'-'*5} {'-'*38} {'-'*38} {'-'*12} {'-'*22} {'-'*22} {'-'*6} {'-'*10}"
                 f" {'-'*22} {'-'*22} {'-'*14}")
    p(f"\n{header}")
    p(separator)

    for i, e in enumerate(events, 1):
        row = (f"{i:<5} {e['encounter']:<38} {e['organization']:<38} {e['host']:<12} {e['server'] or '-':<22}"
    ...
    if events:
        ...
        for ts in sorted(rps_buckets):
            md_lines.append(f"| {ts.strftime('%Y-%m-%d %H:%M:%S')} | {rps_buckets[ts]} |")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze eklaim scale log for latency per encounter, org, and host")
