...
JSON_FILE_PATH_1 = (
    "1_djamil/1_update_encounters_filtered.json"  # First/earlier update file
)
...
def get_db_counts(engine, encounter_id: str) -> dict:
    ...
    with engine.connect() as conn:
        ...
        patient_result = conn.execute(
            patient_query, {"enc_id": encounter_id}
        ).fetchone()
        ...
        for table, filter_col in tables.items():
            if table == "custom_family_member_history":
                if patient_id:
                    query = text(
                        f"SELECT COUNT(*) FROM {table} WHERE {filter_col} = :id"
                    )
                ...
                query = text(
                    f"SELECT COUNT(*) FROM {table} WHERE {filter_col} = :enc_id"
                )
                result = conn.execute(query, {"enc_id": encounter_id}).fetchone()
...
def get_composition_counts_by_type(engine, encounter_id: str) -> dict:
    query = text(
        "SELECT type, COUNT(*) as cnt FROM custom_composition WHERE encounter_id = :enc_id GROUP BY type"
    )
...
def check_encounter_new(update: dict, engine, verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print(
            f"  custom_composition: expected={exp_comp}, actual={act_comp} [{status}]"
        )
    ...
    if verbose:
        ...
        print(
            f"  custom_composition_section: expected={exp_sec}, actual={act_sec} [{status}]"
        )
    ...
    if verbose:
        print("\n" + "=" * 60)
        print(
            f"RESULT: {'All counts match!' if all_match else 'Some counts DO NOT match!'}"
        )
        print("=" * 60)
...
def check_encounter_existing(
    update1: dict, update2: dict, engine, verbose: bool = True
) -> dict:
    ...
    for table in expected_combined:
        ...
        if exp != act:
            all_match = False
            result["mismatches"].append(
                f"{table}: expected={exp} (f2:{expected2[table]}), actual={act}"
            )
        if verbose:
            status = "OK" if exp == act else "MISMATCH"
            print(
                f"  {table}: expected={exp} (f2:{expected2[table]}), actual={act} [{status}]"
            )
    ...
    if verbose:
        status = "OK" if exp_comp == act_comp else "MISMATCH"
        print(
            f"  custom_composition: expected={exp_comp} (f2:{sum(compositions2.values())}), actual={act_comp} [{status}]"
        )
    ...
    if verbose:
        status = "OK" if exp_sec == act_sec else "MISMATCH"
        print(
            f"  custom_composition_section: expected={exp_sec} (f2:{sections2}), actual={act_sec} [{status}]"
        )
...
log_file = open("2a_test_coherence_output.log", "w", encoding="utf-8")
