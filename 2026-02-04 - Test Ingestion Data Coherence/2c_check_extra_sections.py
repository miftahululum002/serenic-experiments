...
JSON_FILE_PATH = "1_djamil/2_update_encounters.json"

# Target parameters
TARGET_NOREGISTRASI = "d44f93d9-3799-c332-1ae3-4707a0ee5f46"
TARGET_SECTION_TABLE_NAME = "custom_observation"  # The table to focus on
...
def format_payload_data(data: dict, table_name: str) -> str:
    ...
    if table_name == 'custom_observation':
        ...
        concept = data.get('concept', {}).get('text') or data.get('concept', {}).get('code', 'N/A')
...
def get_resource_data_from_db(engine, table_name: str, resource_id_in_org: str) -> dict:
    ...
    columns_map = {
        'custom_observation': ['id', 'id_in_organization', 'category', 'concept_code', 'concept_text', 
                               'value_type', 'value_quantity_value', 'value_quantity_unit', 'value_string', 
                               'value_boolean', 'value_integer', 'value_float', 'effective_time', 'created_time'],
        'custom_clinical_impression': ['id', 'id_in_organization', 'category', 'text', 'effective_time', 'created_time'],
        'custom_diagnosis': ['id', 'id_in_organization', 'code', 'diagnosis_text', 'is_primary', 'created_time'],
        'custom_procedure': ['id', 'id_in_organization', 'code', 'procedure_text', 'created_time'],
        'custom_diagnostic_report': ['id', 'id_in_organization', 'category', 'concept_code', 'concept_text', 
                                     'result', 'effective_time', 'created_time'],
        'custom_medication': ['id', 'id_in_organization', 'medication_name', 'dosage_quantity', 'dosage_unit', 'created_time'],
        'custom_service_request': ['id', 'id_in_organization', 'category', 'concept_code', 'concept_text', 'created_time'],
    }
    ...
    query = text(f"SELECT {columns_str} FROM {table_name} WHERE id_in_organization = :id_in_org")
...
def get_composition_sections_from_db(engine, encounter_id: str, table_name: str) -> list:
    ...
    query = text("""
        ...
        WHERE cc.encounter_id = :enc_id AND ccs.section_table_name = :table_name
        ORDER BY ccs.section_sequence_order
    """)
    
    sections = []
    with engine.connect() as conn:
        results = conn.execute(query, {'enc_id': encounter_id, 'table_name': table_name}).fetchall()
        for row in results:
            sections.append({
                'section_id': row[0],
                'table_name': row[1],
...
def get_expected_items_for_table(sources: dict, table_name: str) -> list:
    ...
    table_to_fields = {
        ...
        'custom_family_member_history': [
            ('asesmen_awal', 'riwayat_penyakit_keluarga', 'custom_family_member_history'),
            ('resume_medis', 'riwayat_penyakit_keluarga', 'custom_family_member_history'),
        ],
    }
...
def main():
    ...
    db_sections = get_composition_sections_from_db(engine, encounter_id, TARGET_SECTION_TABLE_NAME)
    ...
    for section in db_sections:
        ...
        db_by_comp_section[key].append({
            'resource_id': section['resource_id_in_org'],
            'section': section
        })
    ...
    for mm in mismatches:
        ...
        print(f"Expected: {mm['expected_count']}, Actual: {mm['db_count']} ({mm['status']} {abs(mm['difference'])})")
        print("=" * 80)
        ...
        for idx, db_item in enumerate(mm['db_items'], 1):
            ...
            if resource_full_id:
                resource_data = get_resource_data_from_db(engine, TARGET_SECTION_TABLE_NAME, resource_full_id)
                if resource_data:
                    formatted = format_resource_data(resource_data, TARGET_SECTION_TABLE_NAME)
                    print(f"[{idx}] {formatted}")
                else:
