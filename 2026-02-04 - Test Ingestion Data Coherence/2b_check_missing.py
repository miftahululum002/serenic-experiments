...
JSON_FILE_PATH_1 = "1_djamil/1_update_encounters.json"
JSON_FILE_PATH_2 = "1_djamil/2_update_encounters.json"

# Target parameters
TARGET_NOREGISTRASI = "7d03cfa0-bac4-8e21-13c2-5aedbcad4584"
TARGET_TABLE = "custom_composition_section"
...
def get_items_from_source(sources: dict, source_name: str, field_name: str | None) -> list:
...
def get_composition_sections_from_db(engine, encounter_id: str) -> list:
    ...
    for section in sections:
        ...
        if table_name and resource_id:
            ...
            id_query = text(f"SELECT id_in_organization FROM {table_name} WHERE id = :res_id")
            with engine.connect() as conn:
...
def get_expected_section_items(sources: dict) -> list:
    ...
    if sources.get('triase_igd'):
        for item in sources['triase_igd']:
            if isinstance(item, dict) and item.get('id'):
                items.append({'id': item['id'], 'source': 'triase_igd', 'table': 'custom_observation'})
    ...
    if sources.get('diagnosis_aktif'):
        for item in sources['diagnosis_aktif']:
            if isinstance(item, dict) and item.get('id'):
                items.append({'id': item['id'], 'source': 'diagnosis_aktif', 'table': 'custom_diagnosis'})
...
def main():
    ...
    if not update2:
        print(f"[ERROR] No update found with noregistrasi: {TARGET_NOREGISTRASI} in File 2")
        return
    ...
    if TARGET_TABLE in TABLES_WITHOUT_ID_IN_ORG:
        ...
        print("Performing comparison via resource IDs (join through composition table).\n")
        ...
        if TARGET_TABLE == 'custom_composition_section':
            ...
            if mismatches:
                for mm in mismatches:
                    ...
                    if mm['table'] in expected_by_table_source:
                        ...
                        for source, count in sorted(expected_by_table_source[mm['table']].items()):
                    ...
                    if mm['table'] in db_by_table_section_type:
                        ...
                        for sec_type, count in sorted(db_by_table_section_type[mm['table']].items()):
            ...
            for table in sorted(all_tables):
                ...
                if table in expected_by_table_source:
                    print(f"  Expected by data source:")
                    for source, count in sorted(expected_by_table_source[table].items()):
                        print(f"    {source}: {count}")
                else:
                ...
                if table in db_by_table_section_type:
                    print(f"  Actual by composition.type.section_type:")
                    for sec_type, count in sorted(db_by_table_section_type[table].items()):
                        print(f"    {sec_type}: {count}")
                else:
