...
def get_has_sep_diagnosis(encounter_id):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT sep_diagnosis FROM encounter WHERE id = %s", (encounter_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def verify_encounter(enc, out=print):
    ...
    has_sep = get_has_sep_diagnosis(encounter_id)
    ...
    for table, field_mappings in RESOURCE_FIELDS.items():
        ...
        if table == "custom_diagnosis" and has_sep:
            expected_count += 1
